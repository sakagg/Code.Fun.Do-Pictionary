...
function guessedcorrectly(indices) {
	...
	playerlist[scorerlist[0]].getElementsByTagName('td')[1].innerHTML = prevScore1 + 3
	playerlist[scorerlist[1]].getElementsByTagName('td')[1].innerHTML = prevScore2 + 2
	if (ismychance == 1) {
		s.send("0")
		ismychance = 0
	}
...
function main() {
	...
	drawcanvas()
	canvas.addEventListener("mousedown", function () { if (ismychance == 1) line() })
